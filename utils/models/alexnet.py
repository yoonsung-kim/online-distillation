...
class SingleManualAlexNet(nn.Module):
    def __init__(self, big_gpu, small_gpu):
        ...
        self.relu_0 = nn.ReLU(inplace=True).to(self.small_gpu)
        ...
        self.relu_1 = nn.ReLU(inplace=True).to(self.small_gpu)
        ...
        self.relu_2 = nn.ReLU(inplace=True).to(self.small_gpu)

        self.conv_3 = nn.Conv2d(384, 256, kernel_size=3, padding=1).to(self.big_gpu)
        self.relu_3 = nn.ReLU(inplace=True).to(self.small_gpu)

        self.conv_4 = nn.Conv2d(256, 256, kernel_size=3, padding=1).to(self.big_gpu)
        self.relu_4 = nn.ReLU(inplace=True).to(self.small_gpu)
        ...
        self.relu_5 = nn.ReLU(inplace=True).to(self.small_gpu)

        # self.dropout_1 = nn.Dropout().to(self.small_gpu)
        self.linear_1 = nn.Linear(4096, 4096).to(self.big_gpu)
        self.relu_6 = nn.ReLU(inplace=True).to(self.small_gpu)
